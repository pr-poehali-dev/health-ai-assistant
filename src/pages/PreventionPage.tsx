...
export default function PreventionPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: '0', role: 'ai', text: t.prevention_greeting, time: getTime() },
  ]);
  ...
  const sendMessage = (text: string) => {
    ...
    setTimeout(() => {
      setIsTyping(false);
      const aiMsg: Message = { id: (Date.now() + 1).toString(), role: 'ai', text: t.prevention_mock, time: getTime() };
    ...
    <div className="flex flex-col bg-background" style={{ height: '100dvh' }}>
      <div className="bg-gradient-to-r from-violet-500 to-purple-600 px-5 safe-top pb-4 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-white font-semibold text-base leading-none">{t.prevention_title}</h1>
            <p className="text-white/70 text-xs mt-0.5">{t.prevention_subtitle}</p>
          </div>
          <div className="ml-auto flex items-center gap-1.5">
            <div className="w-2 h-2 rounded-full bg-green-300 pulse-green" />
            <span className="text-white/70 text-xs">{t.prevention_online}</span>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-2.5 flex-shrink-0 bg-white border-b border-border">
        <div className="flex gap-2 overflow-x-auto scrollbar-hide">
          {(t.prevention_quick as readonly string[]).map((s) => (
            <button
              key={s}
              onClick={() => sendMessage(`${t.prevention_quick_prefix}${s}`)}
      ...
      <div className="px-4 pt-3 bg-white border-t border-border flex-shrink-0 safe-bottom">
        <div className="flex gap-2 items-end mb-16">
          <textarea
            ...
            placeholder={t.prevention_placeholder}
            rows={1}
